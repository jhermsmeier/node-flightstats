...
function filterByAirport( flights, airport, direction ) {
  ...
  return flights.filter( function( flight ) {
    return flight[ dir ].airport.iata === airport ||
      flight[ dir ].airport.icao === airport ||
      flight[ dir ].airport.fs === airport ||
      flight[ dir ].airport.faa === airport
  })
    ...
    return request({
      ...
      if( data != null ) {
        ...
        if( options.airport && data.flights ) {
          data.flights = filterByAirport( data.flights, options.airport, options.direction )
        }
      }
