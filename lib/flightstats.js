...
FlightStats.prototype = {
  ...
  _clientRequest: function( options, callback ) {
    ...
    var query = Object.assign({
      ...
      appKey: this.options.apiKey || this.options.appkey,
    }, options.qs )

    if( options.extendedOptions && options.extendedOptions.length ) {
      query.extendedOptions = options.extendedOptions.join( '+' )
    }
    ...
    debug( 'http:request', options.method || 'GET', url, headers )
    ...
    return request.concat({
      ...
      if( error || res.statusCode != 200 ) {
        ...
        error = error || new Error( res.statusMessage )
      ...
      if( !error && data && data.error ) {
        error = new Error( data.error.errorMessage )
        error.code = data.error.errorCode
      }

      // debug( 'http %s %s', res && res.request.method, res && res.request.url.pathname )
      debug( 'http %s %s', res && res.statusCode, res && res.statusMessage )
      debug( 'http', res && res.headers )
